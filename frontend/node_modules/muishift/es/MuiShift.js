...
var _renderInput = function _renderInput(_ref) {
  ...
  return React.createElement(TextField, _extends({ InputProps: _extends({}, getInputProps()) }, inputProps));
};
...
var _getFilteredItems = function _getFilteredItems(_ref3) {
  ...
  if (keys.length && isTyping) {
    return matchSorter(items, inputValue, _extends({
      keys: keys
    }, matchSorterProps));
  } else {
    return matchSorter(items, inputValue, _extends({}, matchSorterProps));
  }
...
export var MuiShift = function MuiShift(rootProps) {
  ...
  return React.createElement(
    Downshift,
    _extends({}, input, { itemToString: itemToString, selectedItem: input ? input.value : undefined }, rootProps),
